$(function(){
	...
	$("#saveBtnHJYY").on('click',function(){
		...
		app.hjyhData = formdata;
		
		console.log("单位用户增加的参数",formdata);
		
		if(app.hjdwbm&&app.hjrybm){
			
			var hjdwPost =  $.ajax({
				type:"post",				
				url:app.postUrls.updateHJDWXX,
//				url:"http://192.168.199.145:8080/hjgl/updateHJDWXX",
				async:false,
				dataType:'json',
				data:formHJDW
...
//				url:"http://192.168.199.145:8080/hjgl/updateHJRYXX",
				async:false,
				dataType:'json',
				data:formHJRY
			});
			$.when(hjdwPost,hjryPost).then(function(res1, res2){
				
				console.log("修改汇交单位信息返回结果",res1);	
				console.log("修改汇交人员信息返回结果",res2);
				
				if(res1[0].status == "success"&& res2[0].status == "success"){					
			...
			$.ajax({
				...
				success:function(res){
					...
					if(res.hjrybm)app.hjrybm = res.hjrybm;
					if(res.hjdwbm)app.hjdwbm = res.hjdwbm;
					fullfillDropMenu();
	...
	$("#updateBtnHJYY").on('click', function(){
		...
		$("#saveBtnHJYY").removeAttr("disabled");
